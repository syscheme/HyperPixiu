...
class AdviceData(EventData):
    ...
    def hatch(advisorId, symbol, exchange=None, **kwargs) :
        ...
        adv.price       = float(kwargs['price'])
        adv.dirNONE     = float(kwargs['dirNONE'])
        adv.dirLONG     = float(kwargs['dirLONG'])
        adv.dirSHORT    = float(kwargs['dirSHORT'])
        adv.Rdaily      = float(kwargs['Rdaily'])
        adv.Rdstd       = float(kwargs['Rdstd'])
        adv.datetime    = datetime.strptime(kwargs['datetime'], '%Y-%m-%d %H:%M:%S')
        adv.dirString()

        ev = Event(EVENT_ADVICE)
        ev.setData(adv)
        return ev
