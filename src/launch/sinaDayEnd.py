...
if __name__ == '__main__':
    ...
    evMdSource = '/mnt/e/AShareSample/screen-dataset/screen-dataset'
    #################
    ...
    muxPathPatterns={
        'tarNamePat_KL5m' : '%s/%s_KL5m*.json' % (evMdSource, SYMBOL),
        'tarNamePat_MF1m' : '%s/%s_MF1m*.json' % (evMdSource, SYMBOL),
        'tarNamePat_KL1d' : '%s/%s_KL1d*.json' % (evMdSource, SYMBOL),
        'tarNamePat_MF1d' : '%s/%s_MF1d*.json' % (evMdSource, SYMBOL),
    }

    playback   = SinaMux(p, **muxPathPatterns) # = p.createApp(SinaMux, **muxPathPatterns)
