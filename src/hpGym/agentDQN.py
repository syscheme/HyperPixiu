...
class agentDQN(MetaAgent):
    ...
    def __init__(self, gymTrader, **kwargs):
        ...
        self._mainExportHomeDir = self.getConfig('mainHomeDir', None) # this agent work as the main when configured, usually point to a dir under webroot
    ...
    def enableMain(self, homeDir):
        self._mainExportHomeDir = homeDir # this agent work as the main when configured, usually point to a dir under webroot
    ...
    def OnNewFrame(self, frameId, col_state, col_action, col_reward, col_next_state, col_done) :
        ...
        if '/' != self._mainExportHomeDir[-1]: self._mainExportHomeDir +='/'

        try :
            statinfo = os.stat(self._mainExportHomeDir)
        except :
            self._gymTrader.error('agent.OnNewFrame() failed to be the main as exportHome[%s] not exists' % (self._mainExportHomeDir))
            self._mainExportHomeDir = None
            return
        ...
        fn_fit_task = '%stasks/fit_%s.tak' % (self._mainExportHomeDir, brainInst)
        try :
        ...
        target_task_file = '%stasks/fit.tak' % self._mainExportHomeDir
    ...
    def OnRemoteResult_fitTask(self, resultFn) :
        ''' merge the fit-training result generated by the remote subordinate into the current brain
        @resultFn  the full path name of the result file (supporsed to be under self._mainExportHomeDir+'res/'), which is bz2 tarball that consists of
