...
class Sina(BackEnd):
    ...
    def start(self, n=10):
        ...
        if ret and self._mode == self.BROKER_API_ASYNC:
            self._pool = Pool(n)
            self._pool.map_async(self._run, range(n))
    ...
    def stop(self):
        ...
        self.pool.close()
        self.pool.join()
        self.info('Sina stopped')

    #------------------------------------------------
    # overwrite of BackEnd
    #------------------------------------------------    
    def searchKLines(self, scale1min, symbols, linesExpected):
        """API GET"""
        ...
        for s in symbols :
            try:
                response = requests.get(url + "&symbol=" + s, headers=copy(self.DEFAULT_GET_HEADERS), proxies=self._proxies, timeout=self.TIMEOUT)
